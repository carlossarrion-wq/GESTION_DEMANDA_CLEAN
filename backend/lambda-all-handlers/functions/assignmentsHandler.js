...
async function createAssignment(body) {
    ...
    if (data.resourceId) {
        ...
        if (hasDate) {
            ...
            // Get absences for this resource on this date
            const absences = await prisma_1.prisma.absence.findMany({
                where: {
                    resourceId: data.resourceId,
                    startDate: {
                        lte: assignmentDate,
            ...
            const absenceHoursToday = absences.reduce((sum, absence) => sum + Number(absence.hoursPerDay || 0), 0);
            
            // Calculate daily capacity: (monthly capacity / 20 working days) - absences
            const baseDailyCapacity = Math.floor(resource.defaultCapacity / 20);
            const dailyCapacity = baseDailyCapacity - absenceHoursToday;
            
            if (assignedHoursToday + data.hours > dailyCapacity) {
                throw new errors_1.BusinessRuleError(`Assignment would exceed daily resource capacity for ${assignmentDate.toISOString().split('T')[0]}. Available: ${dailyCapacity - assignedHoursToday} hours, Requested: ${data.hours} hours, Assigned: ${assignedHoursToday} hours, Absences: ${absenceHoursToday} hours`, 'DAILY_CAPACITY_EXCEEDED');
            }
        }
    }
            ...
            if (existingAssignment.date || data.date) {
                ...
                if (assignmentDate) {
                    ...
                    if (assignedHoursToday + hoursToUpdate > dailyCapacity) {
                        throw new errors_1.BusinessRuleError(`Assignment would exceed daily resource capacity for ${assignmentDate.toISOString().split('T')[0]}. Available: ${dailyCapacity - assignedHoursToday} hours, Requested: ${hoursToUpdate} hours, Assigned: ${assignedHoursToday} hours, Absences: ${absenceHoursToday} hours`, 'DAILY_CAPACITY_EXCEEDED');
                    }
                }
            }
        }
