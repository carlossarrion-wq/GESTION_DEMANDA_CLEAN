...
const getCapacityOverview = async (userTeam, queryParams) => {
    ...
    const resourcesWithMetrics = resources.map((resource) => {
        ...
        for (let month = 1; month <= 12; month++) {
            ...
            const dailyBaseHours = resource.defaultCapacity / 20;
            const workingDays = getWorkingDaysInMonth(year, month);
            const baseHours = dailyBaseHours * workingDays;
            const absences = assignments.filter((a) => a.projectCode && a.projectCode.startsWith('ABSENCES'));
            const normalAssignments = assignments.filter((a) => !a.projectCode || !a.projectCode.startsWith('ABSENCES'));
